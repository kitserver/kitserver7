...
void bootservAtFaceHairCallPoint();
void bootservAtCopyEditDataCallPoint();
KEXPORT void bootservAtFaceHair(DWORD dest, DWORD src);
...
void bootservReadReplayData(LPCVOID data, DWORD size);
void bootservWriteReplayData(LPCVOID data, DWORD size);
...
HRESULT STDMETHODCALLTYPE initModule(IDirect3D9* self, UINT Adapter,
    ...
    HookCallPoint(code[C_CHECK_FACE_AND_HAIR_ID], 
            bootservAtFaceHairCallPoint, 6, 20);
    ...
    */
    ...
    //addReadReplayDataCallback(bootservReadReplayData);
    //addWriteReplayDataCallback(bootservWriteReplayData);
...
bool bootservGetFileInfo(DWORD afsId, DWORD binId, HANDLE& hfile, DWORD& fsize)
{
    ...
    wstring* pws = _fast_bin_table[binId - FIRST_BOOT_SLOT];
    if (pws) 
    {
